% -- Synchronous Cellular Genetic Algorithm According to Genotype Criterion ----
clear all
...
%% variable used during the calcul ---------------------------------------
     % initialise the vector regrouping the radius
            RADIUS = [];
     % initialise the vector regrouping the ratio
            RATIO = [];
...
mean_entropy_previous = 0;
average_difference_actual = mean_entropy_actual - mean_entropy_previous;
...
for i=0:(grid_y - 1)
    for j=1:grid_x
        ...
        if child_fit <= fit_parent_1
           futur_population(chosen,:) = child;
           futur_fit(chosen) = child_fit;
        end
    end
end
...
threshold_c1 = (1 + epsilon) * average_difference_previous;
threshold_c2 = (2  - epsilon) * average_difference_previous;
% ----- exploit: change towards square shapes ----
if average_difference_actual < threshold_c1
   if grid_y ~= ub_grid_y
...
if average_difference_actual > threshold_c2
   if grid_y ~= lb_grid_y
...
%% ------- Record the ratio and the radius ----------------------------
data = RatioCompute(grid_x,grid_y);
RADIUS = [RADIUS;data(1)];
RATIO =  [RATIO;data(2)];
% -------------------------------------------------------------------------
